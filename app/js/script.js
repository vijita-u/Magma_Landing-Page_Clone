const data_1 = `Magma_Landing-Page_Clone/images/frames00007.png
	...
	Magma_Landing-Page_Clone/images/frames00019.png
	Magma_Landing-Page_Clone/images/frames00022.png
	Magma_Landing-Page_Clone/images/frames00025.png
...
const data_4 = `https://thisismagma.com/assets/home/roadmap/seq/1.webp
...
const hamburger_firstLine = document.querySelector(".first_line");
const hamburger_lastLine = document.querySelector(".last_line");
const navItems = document.querySelector(".navbar__navItems");

hamburgerBtn.addEventListener("click", () => {
	hamburgerBtn.classList.toggle("bgcolor");
	hamburger_firstLine.classList.toggle("first_rotate");
	hamburger_lastLine.classList.toggle("last_rotate");
...
function canvas(page, data) {
	...
	// Creating an array of image tags with resp. image files
	const images = [];
	const imageSequence = {
		frame: 0,
	};

	// Creating <img /> tags & adding it to the images array
...
// page2, page4, page6
const mainText_array = Array.from(document.querySelectorAll(".text_main"));
mainText_array.forEach((mainText) => {
	let clutter = "";
	let text_array = mainText.textContent.split("");

	text_array.forEach((letter) => {
		if (letter !== " ") {
			clutter += `<span class="main__span" style="display: inline-block;"> ${letter} </span>`;
		} else {
			clutter += `<span class="main__span"> ${letter} </span>`;
		}

		mainText.innerHTML = clutter;
	});
});
...
// canvas("#page10", data_4);

function canvas_roadmap(page, data) {
	const canvas = document.querySelector(`${page} canvas`);
	const context = canvas.getContext("2d");
...
canvas_roadmap("#page10", data_4);

gsap.to(".percentCircle", {
	scrollTrigger: {
		trigger: ".percentCircle",
		start: "top center",
		end: "bottom top",
		scroller: "#main",
		scrub: 0.5,
...
gsap.to(".percentCircle__inner", {
	scrollTrigger: {
		trigger: ".percentCircle__inner",
		start: "top center",
		end: "bottom top",
		scroller: "#main",
		scrub: 0.5,
...
gsap.from(".textContainer>a>button", {
	delay: 2,
	y: 50,
	opacity: 0,
	duration: 1,
...
gsap.from(".textContainer__main", {
	delay: 1,
	opacity: 0,
	duration: 2,
	onStart: function () {
		$(".textContainer__main").textillate({
			in: {
...
gsap.from(".whatInfo__title", {
	delay: 1,
	duration: 1,
	onStart: function () {
		$(".whatInfo__title").textillate({
			in: {
				effect: "rotateInUpLeft",
			},
		});
